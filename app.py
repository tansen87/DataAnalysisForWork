...
class MainWindow(QMainWindow, Ui_MainWindow, QtWidgets.QTableView):
    ...
    def merge_data(self):
        ...
        self.thread_merge.signal_desc.connect(self.view_display)
        self.thread_merge.progressBarValue.connect(self.callback)
        self.thread_merge.signal_done.connect(self.callback_done)
...
class ScreenDataThread(QThread):
    ...
    def run(self):
        ...
        self.signal_desc.emit('End of operation')
    ...
    def run(self):
        ...
        try:
            ...
            for file in range(len(files)):
                ...
                self.write(read_tips)
                self.log.write_log(read_tips)
                self.progressBarValue.emit(int(file / len(files) * 100))
