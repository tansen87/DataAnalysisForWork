...
class ScreenDataThread(QThread):
    ...
    def run(self):
        self.signal_desc.emit('Start running...')
        import pandas as pd
        try:
        ...
        self.signal_desc.emit('End of run')
...
class ScreenDataMulThread(QThread):
    signal_trans = Signal(str)
    signal_desc = Signal(str)

    def __init__(self, file_name, targets, column_value, file_out_mer):
        super(ScreenDataMulThread, self).__init__()
        self.log = RecordLog
        self.file_name = file_name
        self.targets = targets
        self.column_value = column_value
    ...
    def run(self):
        ...
        try:
            ...
            data_fr = pd.DataFrame(df, columns=ls_col)
            cd = {
                self.column_value: self.targets
            }
            new_df = self.df_screen(data_fr, cd)
            try:
                new_df.to_excel(f"{str(self.file_out_mer)}.xlsx", encoding='gbk', index=False)
                out = f"{str(self.file_out_mer)}.xlsx is saved."
                self.write(out)
                self.log.write_log(out)
            except:
                new_df.to_csv(f"{str(self.file_out_mer)}.csv", encoding="utf-8", index=False)
                out = f"{str(self.file_out_mer)}.csv is saved."
                self.write(out)
                self.log.write_log(out)
        except Exception as e:
            err = f"Error: {e}"
